...
//		}
	...
	public static String calculateElapsTime(Date time, Context context){
		if(time == null) return context.getString(R.string.last_sync_time_never);
		
		String res ="";
		long diffTime = new Date().getTime() - time.getTime();
		if(diffTime < 60*1000){
			res = context.getString(R.string.last_sync_time_just);
		}else if(diffTime >= 60*1000 && diffTime < 60*60*1000){
			res = context.getString(R.string.last_sync_time_min, String.valueOf(diffTime/(60*1000)));
		}else if(diffTime >= 60*1000 && diffTime < 24*60*60*1000){
			res = context.getString(R.string.last_sync_time_hour, String.valueOf(diffTime/(60*60*1000)));
		}else{
			res = context.getString(R.string.last_sync_time_day, String.valueOf(diffTime/(24*60*60*1000)));
		}
